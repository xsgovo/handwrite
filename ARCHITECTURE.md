...
- 当前产品仅支持单页文档；多页、文件夹等能力等待 UI 需要时再引入领域模型。
...
- 文档至少包含一页。
- Document/Page/Element/Resource 使用稳定本地 `Long ID`。
...
- `:core:document`：文档命令和 Repository 契约
- `:core:data`：资源和持久化实现
...
- 文档写入直接在 Room 事务内完成；不引入写前日志、操作幂等表或启动回放机制。
